...
export default function Home() {
  ...
  const price = {
    monthly: Math.round((config.cpu * 300 + config.ram * 150 + config.storage * 3 + config.traffic * 200) * 100) / 100,
    daily:
      Math.round(((config.cpu * 300 + config.ram * 150 + config.storage * 3 + config.traffic * 200) / 30) * 100) / 100,
    setup: 0,
    total: Math.round((config.cpu * 300 + config.ram * 150 + config.storage * 3 + config.traffic * 200) * 100) / 100,
  }
  ...
  const slides = [
    ...
    {
      title:
        language === "ru" ? "Высокопроизводительные серверы для любых задач" : "High-performance servers for any task",
      description:
        language === "ru"
  ...
  const features = [
    {
      ...
      description:
        language === "ru" ? "Маршрутизация и фильтрация трафика, VPN." : "Traffic routing and filtering, VPN.",
      icon: "shield",
    },
    {
      title: language === "ru" ? "Статические внешние адреса" : "Static External Addresses",
      description: language === "ru" ? "IPv4 и IPv6." : "IPv4 and IPv6.",
      icon: "globe",
    },
    {
      title: language === "ru" ? "Скорость 1000 Мбит/сек" : "1000 Mbps Speed",
      description: language === "ru" ? "Локальная сеть 10 Гбит." : "10 Gbps local network.",
      icon: "zap",
    },
    {
      title: language === "ru" ? "Дата-центр DataPro" : "DataPro Data Center",
      description: language === "ru" ? "Защита от DDoS-атак." : "DDoS protection.",
      icon: "database",
    },
    {
      title: language === "ru" ? "Техническая поддержка 24/7" : "24/7 Technical Support",
      description: language === "ru" ? "Русскоязычная документация." : "Russian documentation.",
      icon: "headphones",
    },
  ]
              ...
              <SwiperSlide key={index}>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-center">
                  ...
                  <div className="relative h-[300px] md:h-[400px]">
                    ...
                    <div className="w-full h-full bg-[#0f1f4b] rounded-lg flex items-center justify-center">
                      <p className="text-white/50">Место для изображения сервера</p>
                    </div>
                  </div>
                </div>
              </SwiperSlide>
      ...
      <div className="py-16 bg-[#0a1a40]">
        <div className="container mx-auto px-4">
          <h2 className="text-3xl md:text-4xl font-bold text-center mb-12">
            {language === "ru" ? "Конфигуратор облачного сервера" : "Cloud Server Configurator"}
          </h2>
          ...
          <div className="grid grid-cols-1 md:grid-cols-12 gap-8">
            <div className="md:col-span-7">
              <div className="bg-[#0f1f4b] rounded-lg p-6">
                <h3 className="text-xl font-bold mb-6">
                  {language === "ru" ? "Настройте параметры вашего сервера" : "Configure your server parameters"}
                </h3>
                  ...
                  <div>
                    <div className="flex items-center justify-between mb-2">
                      <div className="flex items-center gap-2">
                        ...
                        <span>{language === "ru" ? "Процессор (vCPU):" : "Processor (vCPU):"}</span>
                      </div>
                    ...
                    <div className="relative h-2 bg-[#0a1a40] rounded-full">
                      <input
                        ...
                        step="1"
                        value={config.cpu}
                        onChange={(e) => setConfig({ ...config, cpu: Number.parseInt(e.target.value) })}
                  ...
                  <div>
                    ...
                    <div className="relative h-2 bg-[#0a1a40] rounded-full">
                      <input
                        ...
                        step="1"
                        value={config.ram}
                        onChange={(e) => setConfig({ ...config, ram: Number.parseInt(e.target.value) })}
                        className="absolute w-full h-full opacity-0 cursor-pointer"
                      />
                      <div
                        className="absolute h-full bg-[#4f46e5] rounded-full"
                  ...
                  <div>
                    <div className="flex items-center justify-between mb-2">
                      <div className="flex items-center gap-2">
                        ...
                        <span>{language === "ru" ? "Дисковое пространство:" : "Disk space:"}</span>
                      </div>
                    ...
                    <div className="relative h-2 bg-[#0a1a40] rounded-full">
                      <input
                        ...
                        onChange={(e) => setConfig({ ...config, storage: Number.parseInt(e.target.value) })}
                        className="absolute w-full h-full opacity-0 cursor-pointer"
                      />
                      <div
                        className="absolute h-full bg-[#4f46e5] rounded-full"
                  ...
                  <div>
                    <div className="flex items-center justify-between mb-2">
                      <div className="flex items-center gap-2">
                        ...
                        <span>{language === "ru" ? "Трафик:" : "Traffic:"}</span>
                      </div>
                    ...
                    <div className="relative h-2 bg-[#0a1a40] rounded-full">
                      <input
                        ...
                        onChange={(e) => setConfig({ ...config, traffic: Number.parseInt(e.target.value) })}
                        className="absolute w-full h-full opacity-0 cursor-pointer"
                      />
                      <div
                        className="absolute h-full bg-[#4f46e5] rounded-full"
            ...
            <div className="md:col-span-5">
              ...
              >
                <h3 className="text-xl font-bold mb-6">
                  {language === "ru" ? "Ваша конфигурация" : "Your Configuration"}
                </h3>

                <div className="bg-[#0a1a40] p-4 rounded-lg mb-6">
                  <div className="grid grid-cols-2 gap-y-4">
                    <div className="text-white/70">{language === "ru" ? "Процессор:" : "CPU:"}</div>
                    <div className="text-right">{config.cpu} vCPU</div>

                    <div className="text-white/70">{language === "ru" ? "Память:" : "Memory:"}</div>
                    <div className="text-right">{config.ram} GB</div>

                    <div className="text-white/70">{language === "ru" ? "Диск:" : "Disk:"}</div>
                    <div className="text-right">{config.storage} GB SSD</div>

                    <div className="text-white/70">{language === "ru" ? "Трафик:" : "Traffic:"}</div>
                    <div className="text-right">{config.traffic} TB</div>
                  </div>
                </div>

                <div>
                  <h4 className="font-semibold mb-4">{language === "ru" ? "Стоимость" : "Cost"}</h4>

                  <div className="space-y-2 mb-4">
                    <div className="flex justify-between">
                      <span className="text-white/70">{language === "ru" ? "Ежемесячная плата:" : "Monthly fee:"}</span>
                      <span>{price.monthly} ₽</span>
                    </div>

                    <div className="flex justify-between">
                      <span className="text-white/70">{language === "ru" ? "Ежедневная плата:" : "Daily fee:"}</span>
      ...
      <div className="py-16 bg-[#0a1a40]">
        <div className="container mx-auto px-4">
          ...
          <p className="text-white/70 text-center mb-12 max-w-2xl mx-auto">
            {language === "ru" ? "Почему клиенты выбирают НУРИ КОИНОТ" : "Why customers choose NURI KOINOT"}
          </p>
